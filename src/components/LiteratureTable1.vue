...
<script>
    ...
    export default {
        ...
        components: {IndexView, Upload, Delete, Download},
        data() {
            return {
                activeID: '',       // 当前激活的id
                pageSize: 10,       // 每页最大数量
        ...
        methods: {
            ...
            downloadFile() {
                ...
                // 请求接口
                axios.get(
                    url + "?id=" + ids, {"responseType": "blob"}
                ).then(res => {
                    let fileNameCode = res.headers["content-disposition"].split(";filename=")[1];
                    console.log(decodeURI(fileNameCode));
                });
            },
            // 获得行id
            getRowKey(row) {
            ...
            tableCellClick(row, column, cell, event) {
                ...
                if (column.property === "name") {
                    // 新窗口打开pdf 预览
                    const {href} = this.$router.resolve({name: 'pdf', query: {url: row.filepath}})
